...
class RabbitMQService {
  ...
  async connect(retryCount = 0) {
    try {
      logger.warning(`Attempting to connect to RabbitMQ: ${config.rabbitmq.url}`);
      ...
      if (retryCount < config.rabbitmq.maxRetries) {
        ...
        logger.error(`Failed to connect to RabbitMQ. Attempt ${retryCount + 1}/${config.rabbitmq.maxRetries}`);
  ...
  async setupChannel() {
    ...
    await this.channel.assertQueue(`${config.rabbitmq.queue}${config.rabbitmq.dlqSuffix}`, { durable: true });
    await this.channel.bindQueue(config.rabbitmq.queue, config.rabbitmq.exchange, config.rabbitmq.routingKey);
    await this.channel.bindQueue(`${config.rabbitmq.queue}${config.rabbitmq.dlqSuffix}`, config.rabbitmq.exchange, "dlq");
      ...
      async (message) => {
        ...
        try {
          ...
          if (retries < config.rabbitmq.maxRetries) {
            ...
            this.channel.sendToQueue(config.rabbitmq.queue, Buffer.from(content), {
              headers: { "x-retries": retries + 1 },
              persistent: true,
            });
          } else {
            logger.messageToDLQ();
            this.channel.publish(config.rabbitmq.exchange, "dlq", Buffer.from(content), {
